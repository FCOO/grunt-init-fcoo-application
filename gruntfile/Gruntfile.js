...
/* OLD VERSION
	function readFile(filename, isJSON, defaultContents){
		if (grunt.file.exists(filename))
			return isJSON ? grunt.file.readJSON(filename) : grunt.file.read(filename);
		else
			return defaultContents;
	}
...
*/
	...
	var optionBuild = (grunt.option('build') || '').toUpperCase();
	var buildDEV	= optionBuild == 'DEV';
	var buildPROD = optionBuild == 'PROD';
...
//			temp_dist_to_dist_files	=	{ expand: true,	cwd: 'temp_dist',	dest: 'dist'			},	//temp_dist/**/*.* => dist/**/*.*
    ...
    concat: {
	  ...
	  jshint: {
		...
		exec: {
			...
			bower_prod	: 'grunt --build=prod',
			bower_dev		: 'grunt --build=dev'
		},
	...
	if (!buildDEV && !buildPROD){
	  defaultTasks = function() {
		...
		defaultTasks.push( 'clean' ); //clean /temp, /temp_dist, and /dist
		...
		if (isPackage && buildPROD){
			//Rename all src.* to "name".* 
			defaultTasks.push('rename:srcjs_to_namejs');
			if (haveStyleSheet)
				defaultTasks.push('rename:srccss_to_namecss');
		}		
		
		if (isPackage && buildDEV){
			defaultTasks.push( 'copy:temp_dist_to_demo' );	//Copy all files from temp_dist to demo
		}
		
		
		if (buildPROD){
			defaultTasks.push( 'copy:temp_dist_to_dist' );	//Copy all files from temp_dist to dist
		}

		if (isApplication && buildDEV){
		  //TODO - 
		}

		defaultTasks.push( 'clean:temp_dist');
	}

	grunt.registerTask('default', defaultTasks	);
	
	//Alias-tasks
	grunt.registerTask('prod'	,	'exec:bower_prod');
	grunt.registerTask('dev'	, 'exec:bower_dev');
